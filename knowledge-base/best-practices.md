...
- **Use `/btw` for side questions** (2026-06-22) — runs the answer in a dismissible overlay that never enters conversation history; keeps context clean for quick lookups. `[ACTION]` Use `/btw` instead of inline questions during long implementation sessions. [Best practices](https://code.claude.com/docs/en/best-practices)
